...
with open(f'{os.getcwd()}\\config.yml') as file:
    config = yaml.load(file, Loader=yaml.FullLoader)

# Reading the data 
d = pd.read_csv('./data/DAYTON_hourly.csv')
...
deep_learner = DeepModelTS(
    ...
    Y_var='DAYTON_MW',
    lag=config.get('lag'),
    LSTM_layer_depth=config.get('LSTM_layer_depth'),
    epochs=config.get('epochs'),
    train_test_split=config.get('train_test_split') # The share of data that will be used for validation
)
...
# Only applicable if train_test_split in the config.yml > 0
